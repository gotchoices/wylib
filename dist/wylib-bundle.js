...
exports.default = {
  ...
  mounted: function mounted() {
    ...
    this.sites.forEach(function (site) {
      ...
      _this9.$set(site, 'selected', null); //GUI needs to react to this
    ...
    Wyseman.request('_main', 'connect', { stay: true }, function (addr) {
      ...
      if (!addr && _this9.tryEvery && !_this9.tryTimer) {
        _this9.retryConnect();
      } else if (addr) {
        _this9.status = null;
        _this9.retryIn = _this9.tryEvery = CountDown; //Retry if disconnected again
        if (_this9.tryTimer) {
          clearTimeout(_this9.tryTimer);_this9.tryTimer = null;
        }
      }
    });
  } //mounted
...
exports.default = {
  ...
  methods: {
    ...
    defColumns: function defColumns(ev) {
      ...
      var _loop = function _loop(key) {
        var conf = _this8.mlbConfig[key],
            col = _this8.state.grid.columns.find(function (e) {
          return e.field == key;
        });
    ...
    hideCol: function hideCol() {
      ...
      var col = this.state.grid.columns.find(function (e) {
        return e.field == _this9.lastMenu;
      });
    ...
    metaListen: function metaListen() {
      ...
      if (this.state.dbView) Wyseman.register(this.id + 'cv', this.state.dbView, function (data, err) {
        if (err) {
          _this10.top().error(err);return;
        }
        //console.log("Dbp got metadata for:", this.state.dbView, data)
        _this10.viewMeta = data;
        var title = (_this10.wm.t.dbp || '') + ': ' + data.title;
        _this10.$parent.$emit('customize', { title: title, help: _this10.state.dbView + ':\n' + data.help }, 'dbp:' + _this10.state.dbView);
      });
    },
    followMaster: function followMaster() {
      ...
      if (fKeyLink) {
        ...
        fKeyLink.columns.forEach(function (key, idx) {
          var fKeyField = fKeyLink.foreign[idx],
              fValue = _this11.master.get(fKeyField);
  ...
  mounted: function mounted() {
    ...
    this.$nextTick(function () {
      if (_this13.state.edit && _this13.state.edit.posted) _this13.editPosts = 1; //What was posted before we quit
      if (_this13.state.filter && _this13.state.filter.posted) _this13.filtPosts = 1;
      //console.log('Was loaded, reload?', this.id, this.state.loaded, this.state.lastLoad)
      if (_this13.state.loaded > 0) //If state says we had data loaded before, reload now
        _this13.reload();
...
exports.default = {
  ...
  computed: {
    ...
    sourceURL: function sourceURL() {
      ...
      if (domain) return path + '?domain=' + domain;else return this.state.source;
    },
    iconStyle: function iconStyle() {
      return {

  ...
  methods: {
    ...
    exportList: function exportList(sites, cb) {
      ...
      var _loop = function _loop(i) {
        Crypto.exportKey('pkcs8', expKeys[i].priv).then(function (keyData) {
          var k = expKeys[i],
    ...
    exportKeys: function exportKeys(ev) {
      ...
      this.exportList(expKeys, function (keyData) {
        if (keyData.length > 0) _this7.top().input(_this7.wm.conExpFile, function (ans, file) {
    ...
    installKey: function installKey(obj) {
      ...
      for (var keyType in obj) {
        ...
        if (keyType == 'ticket' || keyType == 'login') {
          ...
          if (site.key) Crypto.importKey('pkcs8', Buffer.from(site.key, 'hex'), KeyConfig, true, ['sign']).then(function (priv) {
            site.priv = priv;
